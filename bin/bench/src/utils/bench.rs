...
pub fn check_tx_sim_validity(
    ...
    for (i, tx_hash) in tx_hashes.into_iter().enumerate() {
        ...
        if expected_gas_used != res.gas_used {
            return Err(eyre::eyre!(
                "gas-used mismatch for {tx_hash}: expected {expected_gas_used} found {}",
                res.gas_used
            ));
        }
        if receipt.success != res.success {
            return Err(eyre::eyre!(
                "success mismatch for {tx_hash}: expected {} found {}",
                receipt.success,
                res.success
            ));
        }
    ...
    for (account, touch_counter) in touches.inner().iter() {
        ...
        if native_exe && (touch_counter.non_native > 0) {
            ...
            warn!(
                "invalid touch count for {account:?}: expected all native, found {:.2?}%",
                ratio * 100.
            );
        }
    ...
    if native_exe && (non_native != 0) {
        ...
        let msg = format!(
            "invalid touch count: expected all native, found {:.2?}%",
            ratio * 100.
        );
    ...
    if !native_exe && (non_native != overall) {
        ...
        // todo: will always be wrong for gas guzzlers
        let msg = format!(
            "invalid touch count: expected all non-native, found {:.2?}%",
            ratio * 100.
        );
        if non_native > 0 {
            warn!("{msg}");
