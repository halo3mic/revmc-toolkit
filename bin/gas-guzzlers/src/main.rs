...
use clap::Parser;
use eyre::Result;
use revmc_toolkit_utils::{self as utils, evm::{ProviderFactory, DatabaseEnv}};
...
fn main() -> Result<()>{
    ...
    let gas_guzzlers = find_gas_guzzlers(
        args.start_block,
        args.end_block,
        args.sample_size,
    )?;
    let parsed = parse_gas_guzzlers(
        gas_guzzlers,
        args.take,
        args.gas_limit,
    );
...
fn parse_gas_guzzlers(
    ...
    match (max_len, max_csum_prop_gas_used) {
        (Some(max_len), Some(max_csum_prop_gas_used)) => {
            giter
                .take(max_len)
        ...
        (None, Some(max_csum_prop_gas_used)) => {
            giter
                .take_while(|e| e.csum_prop_gas_used < max_csum_prop_gas_used)
                .collect()
        }
