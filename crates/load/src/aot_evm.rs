...
// todo: it would be useful to track touches to check all compiled fncs are actually used instead of native, but this can be done by a wrapper around ExternalContext
...
{    
    ...
    //     ctx.external.as_ref().register_call_touch(inputs.bytecode_address);
    ...
    handler.execution.execute_frame = Arc::new(move |frame, memory, tables, context| {
        ...
        // let ext_ctx = context.external.as_ref();
        let ext_fn = context.external.as_ref().get_function(bytecode_hash);   

        // ext_ctx.register_touch(
